...
router.post('/', upload.single('billImage'), async (req, res) => {
  ...
  // Map từ Flutter sang backend field
  const {
    userId,
    accountId,
    categoryName,
  ...
  const danhMucId = 1; // TODO: bạn có thể map categoryName sang id từ bảng danh_muc nếu muốn
  ...
  if (isNaN(parsedAmount)) {
    console.log('Số tiền không hợp lệ:', soTien);
...
router.put('/:id', upload.single('billImage'), async (req, res) => {
  ...
  const danhMucId = 1; // TODO: map categoryName sang id nếu muốn
  const anhHoaDon = req.file ? `/uploads/${req.file.filename}` : null;
