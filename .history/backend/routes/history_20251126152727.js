...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    const [bankRows] = await conn.execute('SELECT * FROM tai_khoan WHERE id = ?', [deletedTransaction.bank_id]);
    ...
    // 3️⃣ Hoàn tiền
    const amount = parseFloat(deletedTransaction.so_tien);
    if (deletedTransaction.loai === "Thu") {
      newBalance -= amount;
    } else {
      newBalance += amount;
    }
    await conn.execute('UPDATE tai_khoan SET so_du=? WHERE id=?', [newBalance, deletedTransaction.bank_id]);
