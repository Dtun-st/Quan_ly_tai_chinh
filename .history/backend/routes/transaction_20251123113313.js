...
router.get("/", async (req, res) => {
  try {
    ...
    const [rows] = await db.execute("SELECT * FROM giao_dich WHERE userId=?", [userId]);
    res.json(rows);
  } catch (err) {
    console.error(err);
...
router.post("/", upload.single("anh_hoa_don"), async (req, res) => {
  try {
    ...
    await db.execute(
      "INSERT INTO giao_dich(userId, tai_khoan_id, danh_muc_id, loai_gd, so_tien, mo_ta, ngay_giao_dich, anh_hoa_don) VALUES(?,?,?,?,?,?,?,?)",
