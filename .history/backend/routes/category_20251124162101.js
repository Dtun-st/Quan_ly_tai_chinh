...
const db = require("../db");
// ===============================
// 📌 Lấy danh mục theo user + (tuỳ chọn) loại
// ===============================
router.get("/:userId", (req, res) => {
  const userId = req.params.userId;
  const type = req.query.type; // Thu | Chi

  let sql = "SELECT * FROM danh_muc WHERE nguoi_dung_id = ?";
  let params = [userId];

  // Nếu có loại (Thu/Chi) thì thêm điều kiện
  if (type) {
    sql += " AND loai = ?";
    params.push(type);
  }
...
// ===============================
router.post("/", (req, res) => {
  const { ten, loai, nguoi_dung_id } = req.body;
  ...
  const sql =
    "INSERT INTO danh_muc (ten, loai, nguoi_dung_id) VALUES (?, ?, ?)";

  db.query(sql, [ten, loai, nguoi_dung_id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Lỗi thêm dữ liệu", details: err });
...
router.delete("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Không thể xoá danh mục", details: err });
