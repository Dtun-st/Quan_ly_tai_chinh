...
router.get('/month', async (req, res) => {
  ...
  if (!userId || !month || !year) {
    return res.status(400).json({ success: false, message: 'Thiếu tham số (userId, month, year)' });
  }
...
// ====================== C. LỌC THEO KHOẢNG THỜI GIAN ======================
router.get('/range', async (req, res) => {
  const { userId, start, end } = req.query;

  if (!userId || !start || !end) {
    return res.status(400).json({ success: false, message: 'Thiếu tham số (userId, start, end)' });
  }
  ...
  try {
    ...
    console.error("Range filter error:", err);
    res.status(500).json({ success: false, message: 'Lỗi server' });
  }
});
