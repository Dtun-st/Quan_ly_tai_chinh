...
router.post('/', upload.single('billImage'), async (req, res) => {
  ...
  if (!userId || !taiKhoanId || !danhMucId || !loaiGd || !soTien || !ngayGd) {
    console.warn('Thiếu thông tin:', { userId, taiKhoanId, danhMucId, loaiGd, soTien, ngayGd });
    return res.status(400).json({ success: false, message: 'Thiếu thông tin' });
  }

  const parsedAmount = parseFloat(soTien);
  if (isNaN(parsedAmount)) {
    console.warn('Số tiền không hợp lệ:', soTien);
  ...
  try {
    ...
    console.log('Insert giao_dich result:', insertResult);
    ...
    console.log('Số dư trước giao dịch:', currentBalance);
    ...
    console.log('Số dư sau cập nhật:', currentBalance);

    await conn.commit();
    res.json({ success: true, message: 'Giao dịch lưu thành công' });
  } catch (err) {
    await conn.rollback();
    console.error('❌ Lỗi trong POST /transaction:', err);
    res.status(500).json({ success: false, message: 'Lỗi server' });
  } finally {
    conn.release();
  }
...
router.put('/:id', upload.single('billImage'), async (req, res) => {
  ...
  try {
    ...
    console.log('Hoàn tác số dư cũ:', oldBalance);
    ...
    console.log('Số dư mới sau cập nhật:', newBalance);

    await conn.commit();
    res.json({ success: true, message: 'Cập nhật thành công' });
  } catch (err) {
    await conn.rollback();
    console.error('❌ Lỗi trong PUT /transaction/:id:', err);
    res.status(500).json({ success: false, message: 'Lỗi server' });
  } finally {
    conn.release();
  }
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    const trans = rows[0];
    console.log('Giao dịch xóa:', trans);
    ...
    console.log('Số dư sau khi xóa:', balance);

    await pool.execute('DELETE FROM giao_dich WHERE id=?', [id]);
    res.json({ success: true, message: 'Đã xóa giao dịch' });
  } catch (err) {
    console.error('❌ Lỗi trong DELETE /transaction/:id:', err);
    res.status(500).json({ success: false, message: 'Lỗi server' });
  }
});
