...
async function insertDefaultCategories(userId) {
  ...
  for (const cat of defaultCategories.filter(c => !c.parentName)) {
    ...
    console.log(`Chèn danh mục cha: ${cat.name} (${cat.type}) -> id=${result.insertId}`);
  }
  ...
  for (const cat of defaultCategories.filter(c => c.parentName)) {
    ...
    console.log(`Chèn danh mục con: ${cat.name} (${cat.type}) -> parentId=${parentId}`);
  }
}
