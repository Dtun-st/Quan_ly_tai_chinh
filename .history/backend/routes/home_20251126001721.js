...
router.post('/ngan_sach', async (req, res) => {
  ...
  try {
    // Kiểm tra user đã có ngân sách chưa
    const [rows] = await pool.execute(
      'SELECT * FROM ngan_sach WHERE nguoi_dung_id = ?',
      [userId]
    );
    ...
    if (rows.length > 0) {
      // Cập nhật
      await pool.execute(
        'UPDATE ngan_sach SET budget = ? WHERE nguoi_dung_id = ?',
        [budget, userId]
      );
    } else {
      // Thêm mới
      await pool.execute(
        'INSERT INTO ngan_sach (nguoi_dung_id, budget) VALUES (?, ?)',
        [userId, budget]
      );
    }
...
router.get('/ngan_sach/:userId', async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(
      'SELECT budget FROM ngan_sach WHERE nguoi_dung_id = ?',
      [userId]
    );
    if (rows.length > 0) {
