...
async function insertDefaultCategories(userId) {
  ...
  console.log(`=== Bắt đầu chèn danh mục mặc định cho userId=${userId} ===`);
  ...
  for (const cat of defaultCategories.filter(c => !c.parentName)) {
    try {
      ...
      console.log(`Đã chèn danh mục cha: ${cat.name} (${cat.type}) -> id=${result.insertId}`);
    } catch (err) {
      console.error(`Lỗi khi chèn danh mục cha ${cat.name}:`, err.message);
    }
  }
  ...
  for (const cat of defaultCategories.filter(c => c.parentName)) {
    try {
      ...
      if (!parentId) {
        console.warn(`⚠️ Không tìm thấy parentId cho danh mục con ${cat.name} (parentName=${cat.parentName})`);
        continue;
      }
      const [result] = await pool.execute(
        'INSERT INTO danh_muc (ten_danh_muc, loai, icon, nguoi_dung_id, parent_id) VALUES (?, ?, ?, ?, ?)',
      ...
      console.log(`Đã chèn danh mục con: ${cat.name} (${cat.type}) -> id=${result.insertId}, parentId=${parentId}`);
    } catch (err) {
      console.error(`Lỗi khi chèn danh mục con ${cat.name}:`, err.message);
    }
  }

  console.log('=== Kết thúc chèn danh mục mặc định ===');
}
