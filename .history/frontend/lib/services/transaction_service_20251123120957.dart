...
class TransactionService {
  ...
  static Future<List<dynamic>> getTransactions(int userId) async {
    final res = await http.get(Uri.parse("${ApiConfig.baseUrl}/transactions?userId=$userId"));
  ...
  /// Thêm giao dịch mới
  /// [imageFile] dùng cho Mobile/Desktop, [imageBytes] dùng cho Web
  static Future<bool> saveTransaction({
    ...
    if (imageFile != null) {
      request.files.add(await http.MultipartFile.fromPath('anh_hoa_don', imageFile.path,
          filename: basename(imageFile.path)));
    } else if (imageBytes != null && imageName != null) {
      request.files.add(http.MultipartFile.fromBytes('anh_hoa_don', imageBytes, filename: imageName));
    }
  ...
  static Future<bool> deleteTransaction(int transactionId) async {
    final res = await http.delete(Uri.parse("${ApiConfig.baseUrl}/transactions/$transactionId"));
