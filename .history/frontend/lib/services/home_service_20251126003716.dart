...
class HomeService {
  ...
  Future<double?> fetchBudget(int userId) async {
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      if (data['budget'] != null) return double.tryParse(data['budget'].toString());
    }
    return null;
  }

  // Lưu ngân sách
  Future<bool> saveBudget(int userId, double soTien) async {
    final url = Uri.parse('${ApiConfig.baseUrl}/api/home/ngan_sach');
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'userId': userId, 'so_tien': soTien}),
    );
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
