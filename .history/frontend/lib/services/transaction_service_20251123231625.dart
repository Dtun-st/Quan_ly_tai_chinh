...
//     String? imageName,
//   }) async {
//     var uri = Uri.parse("${ApiConfig.baseUrl}/api/transaction");
...
//       Uri.parse("${ApiConfig.baseUrl}/api/transaction/$transactionId"),
//     );
...
class TransactionService {
  static String? selectedChiDanhMuc;
  static String? selectedChiCon;
  static String? selectedThuLoai;

  static String getSelectedCategoryText(bool isChi) {
    if (isChi) {
      return selectedChiCon ?? "Chọn danh mục";
    }
    return selectedThuLoai ?? "Chọn loại thu";
  }

  static Future<List<dynamic>> getTransactions(int userId) async {
    final res = await http.get(Uri.parse("http://localhost:3000/api/transactions/$userId"));
    if (res.statusCode == 200) {
  ...
  static Future<bool> saveTransaction({
    ...
    if (accountId == null || amount == null || selectedCategory == null) {
      return false;
    }

    final uri = Uri.parse("http://localhost:3000/api/transactions");
