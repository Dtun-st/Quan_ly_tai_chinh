...
class CategoryService {
  ...
  static Future<List<dynamic>> getCategories(String type) async {
    try {
      ...
      final url = Uri.parse("${ApiConfig.baseUrl}/api/categories/$userId?type=$type");
      final response = await http.get(url);
  ...
  // 📌 Thêm danh mục
  // ================================
  static Future<bool> addCategory(String name, String type, String icon, {int? parentId}) async {
    try {
      final userId = await _getUserId();
  ...
  // 📌 Sửa danh mục
  // ================================
  static Future<bool> updateCategory(int id, String name, String icon) async {
    try {
      final url = Uri.parse("${ApiConfig.baseUrl}/api/categories/$id");
      final response = await http.put(
        url,
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({"ten": name, "icon": icon}),
      );
      return response.statusCode == 200;
    } catch (e) {
