...
class BankService {
  ...
  Future<List<Map<String, dynamic>>> getAccounts() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true && data['accounts'] != null) {
          ...
          print("⚠️ API trả về success=false hoặc accounts=null: ${data['message']}");
        }
      } else {
        print("❌ HTTP ${response.statusCode}: ${response.body}");
      }
    ...
    // Fallback tạm thời nếu API fail
    return [
      {"id": 1, "name": "Tiền mặt", "icon": "money"},
      {"id": 2, "name": "Vietcombank", "icon": "account_balance"},
    ];
  }
}
