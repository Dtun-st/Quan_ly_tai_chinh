...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  IconData _getIcon(String? iconCode) {
    if (iconCode == null || iconCode.isEmpty) {
      return Icons.category; // icon mặc định nếu null
    }
    return IconData(int.tryParse(iconCode) ?? Icons.category.codePoint,
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            child: ExpansionTile(
              leading: Icon(_getIcon(parent['icon']), size: 28),
              title: Text(parent['name'] ?? 'Tên trống',
                  style: const TextStyle(
                      fontSize: 18, fontWeight: FontWeight.bold)),
              children: [
                ...List<Widget>.from((parent['children'] ?? []).map((child) {
                  return ListTile(
                    leading: Icon(_getIcon(child['icon'])),
                    title: Text(child['name'] ?? 'Tên trống'),
                  );
                })),
                TextButton.icon(
                  onPressed: () => _addChildCategory(parent['id']),
