...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadInitialData() async {
    try {
      // Load dữ liệu tài khoản, danh mục chi/thu, giao dịch từ service
      final accs = await TransactionService.getAccounts();
      final chiCats = await TransactionService.getChiCategories();
      final thuCats = await TransactionService.getThuCategories();
      final txs = await TransactionService.getTransactions(1); // userId tạm 1
  ...
  Future<void> saveTransaction() async {
    ...
    bool success = await TransactionService.saveTransaction(
      userId: 1,
      accountId: selectedAccountId!,
      categoryId: isChi ? 1 : 0, // TODO: map từ danh mục đã chọn
      type: isChi ? "Chi" : "Thu",
      amount: double.tryParse(amountCtrl.text) ?? 0,
      description: descCtrl.text,
      date: selectedDate,
    ...
    if (success) {
      ...
      _loadInitialData(); // tải lại danh sách sau khi thêm
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
