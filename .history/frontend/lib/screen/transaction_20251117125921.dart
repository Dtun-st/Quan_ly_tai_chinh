...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  final List<Map<String, dynamic>> chiDanhMuc = [
    ...
    },
    {
      ...
      "children": ["Học phí", "Quần áo", "Đồ chơi"]
    },
  ];
  ...
  Widget _buildThuForm() {
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          children: thuLoai.map((t) {
            bool isSelected = selectedThuLoai == t['name'];
            return ChoiceChip(
              label: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(t['icon'], size: 18, color: isSelected ? Colors.white : Colors.black),
                  const SizedBox(width: 4),
                  Text(t['name'], style: TextStyle(color: isSelected ? Colors.white : Colors.black)),
                ],
              ),
              selected: isSelected,
              selectedColor: Colors.orange,
              backgroundColor: Colors.grey.shade200,
              onSelected: (_) => setState(() => selectedThuLoai = t['name']),
            );
          }).toList(),
        ),
        const SizedBox(height: 12),
  ...
  // Danh mục + chi tiết Chi
  Widget _buildChiDanhMucDropdown() {
    return Column(
        ...
        return Column(
          ...
          children: [
            Container(
              ...
              child: Row(
                children: [
                  Icon(cat['icon'], color: Colors.orange),
                  const SizedBox(width: 8),
            ...
            Wrap(
              spacing: 8,
              children: cat['children'].map<Widget>((c) {
                bool isSelected = selectedChiCon == c;
                return ChoiceChip(
                  label: Text(c),
                  selected: isSelected,
                  selectedColor: Colors.orange,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return Card(
          margin: const EdgeInsets.symmetric(vertical: 4),
          child: ListTile(
            leading: Icon(isChi ? Icons.remove : Icons.add, color: isChi ? Colors.red : Colors.green),
            title: Text(isChi
                ? '150,000 VND - ${selectedChiCon ?? "Nhà hàng"}'
                : '5,000,000 VND - ${selectedThuLoai ?? "Lương"}'),
            subtitle: Text('Ngày: ${selectedDate.day}/${selectedDate.month}/${selectedDate.year}'),
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
