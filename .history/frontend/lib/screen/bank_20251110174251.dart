...
class _BankScreenState extends State<BankScreen> {
  ...
  Future<void> loadAccounts() async {
    final data = await bankService.getAccounts(1); // giả sử userId = 1
    setState(() {
      accounts = data;
      totalBalance = accounts.fold(
          0, (sum, acc) => sum + (acc['so_du'] as num).toDouble());
    });
  }

  void openEditDialog(Map<String, dynamic> acc) {
    TextEditingController nameController = TextEditingController(text: acc["ten_tai_khoan"]);
    TextEditingController balanceController = TextEditingController(text: acc["so_du"].toString());
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              final success = await bankService.updateAccount(acc["id"], {
                "ten_tai_khoan": nameController.text,
                "so_du": double.tryParse(balanceController.text) ?? acc["so_du"],
              });
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              final success = await bankService.deleteAccount(acc["id"]);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: Colors.orange,
        onPressed: () {
          // TODO: mở form thêm tài khoản (gọi bankService.addAccount)
        },
        child: const Icon(Icons.add),
      ),
      body: RefreshIndicator(
  ...
  Widget buildAccountCard(Map<String, dynamic> acc) {
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: Colors.orange.shade100,
        child: Icon(Icons.account_balance_wallet, color: Colors.orange.shade700),
      ),
      title: Text(acc['ten_tai_khoan']),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(isHidden ? "*****" : "${acc['so_du']} đ"),
          IconButton(
            icon: const Icon(Icons.edit, color: Colors.blue),
            onPressed: () => openEditDialog(acc),
          ),
