...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadUserIdAndTransactions() async {
    ...
    userId = prefs.getInt('userId') ?? 1; // mặc định 1 nếu chưa set
  ...
  Widget _buildTransactionList() {
    ...
    return Column(
      children: transactions.map((tx) {
        ...
        Widget? imgWidget;
        if (tx['image_url'] != null) {
          imgWidget = kIsWeb
              ? Image.network(tx['image_url'], width: 50, height: 50, fit: BoxFit.cover)
              : Image.file(File(tx['image_url']), width: 50, height: 50, fit: BoxFit.cover);
        }
        ...
        return Card(
          ...
          child: ListTile(
            ...
            trailing: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(amountText, style: const TextStyle(fontWeight: FontWeight.bold, color: primaryColor)),
                const SizedBox(height: 4),
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: const [
