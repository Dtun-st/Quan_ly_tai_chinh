...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadTransactions() async {
    ...
    final allTx = await _service.getTransactions(nguoiDungId: currentUserId);
    setState(() {
      _transactions = allTx.reversed.toList();
    });
  }
  ...
  void _editTransaction(Map<String, dynamic> tx) {
    setState(() {
      editingTransactionId = tx['id'];
      selectedAccountId = tx['taiKhoanId'];
      selectedCategoryId = tx['hanMucId'];
      type = tx['loaiGd'];
      _amountCtrl.text = (tx['soTien'] ?? 0).toString();
      _descCtrl.text = tx['moTa'] ?? '';
      selectedDate = DateTime.parse(tx['ngayGiaoDich']);
    });
  }
        ...
        child: Column(
          ...
          children: [
                    ...
                    items: categories
                        .map(
                          (c) => DropdownMenuItem<int>(
                            value: c['id'],
                            child: Row(
                              children: [Icon(c['icon']), const SizedBox(width: 8), Text(c['name'])],
                            ),
                          ),
                        )
                        .toList(),
            ...
            if (_transactions.isEmpty)
              ...
              Column(
                children: _transactions.map((tx) {
                  final date = DateTime.parse(tx['ngayGiaoDich']);
                  final categoryName = categories.firstWhere((c) => c['id'] == tx['hanMucId'], orElse: () => {"name": "Khác"})['name'];
                  final color = categoryColors[categoryName] ?? Colors.grey;

                  final amount = _currencyFormat.format((tx['soTien'] ?? 0));

                  return Card(
                    margin: const EdgeInsets.symmetric(vertical: 4),
                    child: ListTile(
                      leading: Icon(tx['loaiGd'] == 'Chi' ? Icons.remove : Icons.add, color: color),
                      title: Text('$amount VND - ${tx['moTa'] ?? ''}'),
                      subtitle: Text('Ngày: ${date.day}/${date.month}/${date.year}'),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _editTransaction(tx)),
                          IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteTransaction(tx['id'])),
                        ],
                      ),
                    ),
                  );
