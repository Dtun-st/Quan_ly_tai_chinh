...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void _saveTransaction() async {
    ...
    final success = await TransactionService().addTransaction(
      userId: 1, // thay bằng userId thật (SharedPreferences hoặc state)
      accountId: selectedAccountId!,
      type: isChi ? "Chi" : "Thu",
      categoryName: isChi
          ? (selectedChiCon?['ten_danh_muc'] ?? "Khác")
          : (selectedThuLoai?['ten_danh_muc'] ?? "Khác"),
      amount: amountCtrl.text,
      description: descCtrl.text,
      date: "${selectedDate.toIso8601String()}",
      imageFile: selectedImageFile, // nếu chạy mobile
    );
