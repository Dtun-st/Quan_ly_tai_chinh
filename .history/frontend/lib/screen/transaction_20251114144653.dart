...
class TransactionScreen extends StatefulWidget {
  final int currentUserId; // ID người dùng hiện tại
  const TransactionScreen({super.key, required this.currentUserId});
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadTransactions() async {
    final allTx = await _service.getTransactions(nguoiDungId: widget.currentUserId);
    setState(() {
      _transactions = allTx.reversed.take(5).toList();
    });
  }
