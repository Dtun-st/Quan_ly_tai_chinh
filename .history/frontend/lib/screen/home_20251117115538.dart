...
// MÀU SẮC MỚI (DỊU MẮT)
// ===================
const Color primaryColor = Color(0xFFFFA726); // Cam nhạt
const Color accentColor = Color(0xFFFFF8E1); // Trắng kem nhạt
const Color highlightColor = Color(0xFFFF6D00); // Cam đậm nhẹ, dùng cho icon / nổi bật
...
class HomePage extends StatelessWidget {
  ...
  // DỮ LIỆU CỨNG
  // ===================
  final List<Map<String, dynamic>> taiKhoan = const [
  ...
  final List<Map<String, dynamic>> giaoDich = const [
    {"han_muc": "Ăn uống", "so_tien": 50000, "loai": "Chi", "ngay": "2025-10-24"},
    {"han_muc": "Lương", "so_tien": 10000000, "loai": "Thu", "ngay": "2025-10-23"},
    {"han_muc": "Đi lại", "so_tien": 200000, "loai": "Chi", "ngay": "2025-10-22"},
  ];

  final List<Map<String, String>> thongBao = const [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: _buildUserHeader(context),
        actions: [
  ...
  Widget _buildUserHeader(BuildContext context) {
    return GestureDetector(
      ...
      child: Row(
        children: [
          CircleAvatar(radius: 20, backgroundColor: Colors.white, child: Icon(Icons.person, color: highlightColor, size: 30)),
          const SizedBox(width: 12),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text("Tổng số dư hiện có", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.white70)),
          const SizedBox(height: 8),
          Text("${_formatCurrency(tongSoDu)} VND", style: const TextStyle(fontSize: 30, fontWeight: FontWeight.bold, color: Colors.white)),
          const SizedBox(height: 10),
          GestureDetector(
            onTap: () => print("Xem báo cáo chi tiết"),
            child: Row(mainAxisAlignment: MainAxisAlignment.end, children: const [
              Text("Xem báo cáo chi tiết", style: TextStyle(color: Colors.white70, fontSize: 14)),
              SizedBox(width: 4),
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Column(children: [
              Icon(
                label == "Thu" ? Icons.arrow_downward : (label == "Chi" ? Icons.arrow_upward : Icons.balance),
                color: color,
                size: 20,
              ),
              const SizedBox(height: 4),
    ...
    return Card(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            leading: CircleAvatar(backgroundColor: highlightColor.withOpacity(0.1), child: Icon(tk['icon'] as IconData?, color: highlightColor)),
            title: Text(tk["ten"], style: const TextStyle(fontWeight: FontWeight.w600)),
            trailing: Text("${_formatCurrency(tk["so_du"].toDouble())} VND", style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: textColor)),
            onTap: () => print("Chi tiết tài khoản ${tk['ten']}"),
          );
        },
      ),
    );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          Text(label, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: textColor)),
        ]),
      ),
    );
  }

  Widget _buildTitle(String title) => Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor));
      ...
      children: giaoDich.map((gd) {
        ...
        return Padding(
          ...
          child: Card(
            ...
            elevation: 1.5,
            child: ListTile(
              leading: CircleAvatar(radius: 20, backgroundColor: color.withOpacity(0.1), child: Icon(isThu ? Icons.trending_up : Icons.trending_down, color: color)),
              ...
              trailing: Text("${isThu ? '+' : '-'}${_formatCurrency(gd['so_tien'].toDouble())} VND", style: TextStyle(fontWeight: FontWeight.bold, color: color, fontSize: 15)),
              onTap: () => print("Chi tiết giao dịch ${gd['han_muc']}"),
            ),
          ),
        );
      }).toList(),
        ...
        return Padding(
          ...
          child: Card(
            ...
            child: ListTile(
              ...
              onTap: () => print("Chi tiết thông báo ${tb['loai']}"),
            ),
          ),
        );
      }).toList(),
