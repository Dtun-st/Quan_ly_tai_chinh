...
//       context,
//       MaterialPageRoute(
//         builder: (_) => const CategoryScreen(title: "Chi tiêu"),
//       ),
//     );
//     if (result != null) {
//       setState(() {
...
//       context,
//       MaterialPageRoute(
//         builder: (_) => const CategoryScreen(title: "Thu nhập"),
//       ),
//     );
//     if (result != null) {
//       setState(() {
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadAccounts() async {
    final list = await _bankService.getAccounts();
    setState(() {
      accounts = list;
        ...
        return DropdownMenuItem<int>(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text("${account['ten_tai_khoan']} (${account['so_du']} đ)"),
            ],
          ),
        );
      }).toList(),
      ...
      child: ElevatedButton.icon(
        onPressed: () async {
          ...
          final transactionData = {
            "user_id": 1, // sau này lấy từ SharedPreferences
            "loai_gd": isChi ? "Chi" : "Thu",
            "so_tien": double.tryParse(amountCtrl.text) ?? 0,
            "danh_muc": isChi ? selectedChiCon : selectedThuLoai,
            "tai_khoan_id": selectedAccountId,
            "ngay_giao_dich": selectedDate.toIso8601String(),
            "anh_hoa_don": null,
          };

          // TODO: Gọi API thêm giao dịch
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Đã lưu giao dịch (test UI)")),
          );
        },
        icon: const Icon(Icons.check),
        label: const Text("LƯU GIAO DỊCH"),
