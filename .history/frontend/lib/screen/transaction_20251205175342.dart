...
//       danhMucId: isChi
//           ? (selectedChiCon != null ? selectedChiCon!['id'] : 53)
//           : (selectedThuLoai != null ? selectedThuLoai!['id'] : 60),
//       type: isChi ? "Chi" : "Thu",
...
//         _showBudgetWarning(result?['percent']);
//       }
...
//   void _showBudgetWarning(percent) {
//     showDialog(
//       context: context,
//       builder: (_) => AlertDialog(
//         title: const Text("⚠ Cảnh báo ngân sách"),
//         content: Text("Bạn đã đạt $percent% ngân sách tháng này!"),
//         actions: [
//           TextButton(
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _saveTransaction() async {
    ...
    if (result?['success'] == true) {
      if (result?['warning'] == true) {
        _showBudgetAlert(result?['percent']);
        _showBannerWarning(result?['percent']);
      }
  ...
  /// ⚠ Hiển thị cảnh báo dạng banner ở top màn hình
  void _showBannerWarning(percent) {
    ScaffoldMessenger.of(context).showMaterialBanner(
      MaterialBanner(
        backgroundColor: Colors.red.shade100,
        content: Text(
          "⚠ Chi tiêu đã đạt $percent% ngân sách!",
  ...
  Widget _buildImagePicker() {
    final content = kIsWeb
        ? (selectedImageBytes == null
            ? const Icon(Icons.camera_alt_rounded, size: 40, color: Colors.grey)
            : Image.memory(selectedImageBytes!, fit: BoxFit.cover))
        : (selectedImageFile == null
            ? const Icon(Icons.camera_alt_rounded, size: 40, color: Colors.grey)
            : Image.file(selectedImageFile!, fit: BoxFit.cover));
