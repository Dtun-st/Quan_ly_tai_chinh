...
//   int? userId; // ✅ không còn gán cứng 1

//   @override
//   void initState() {
//     super.initState();
//     loadUserIdAndAccounts();
//   }

//   Future<void> loadUserIdAndAccounts() async {
//     final prefs = await SharedPreferences.getInstance();
//     userId = prefs.getInt('userId'); // lấy userId từ login
//     if (userId != null) {
//       await loadAccounts();
//     } else {
...
//     final list = await service.getAccounts(); // ✅ bỏ userId
//     setState(() {
//       accounts = list;
//       totalBalance =
//           accounts.fold(0, (sum, acc) => sum + _parseDouble(acc['so_du']));
//     });
//   }
...
//             isHidden
//                 ? "*****"
//                 : "${_parseDouble(acc["so_du"]).toStringAsFixed(0)} đ",
//             style: TextStyle(
//               fontSize: 16,
//               fontWeight: FontWeight.bold,
//               color:
//                   _parseDouble(acc["so_du"]) < 0 ? Colors.red : Colors.black,
//             ),
//           ),
...
class _BankScreenState extends State<BankScreen> {
  ...
  Future<void> _loadAccounts() async {
    ...
    setState(() {
      ...
      totalBalance = accounts.fold(
          0, (sum, acc) => sum + (acc['so_du'] != null ? acc['so_du'].toDouble() : 0));
    });
  }
  ...
  void _openAccountForm({Map<String, dynamic>? account}) {
    final nameController =
        TextEditingController(text: account?['ten_tai_khoan'] ?? '');
    final balanceController = TextEditingController(
        text: account != null ? account['so_du'].toString() : '');
    String selectedType = account?['loai_tai_khoan'] ?? 'Tiền mặt';
  ...
  Widget _buildAccountCard(Map<String, dynamic> acc) {
    double balance = acc['so_du'] != null ? acc['so_du'].toDouble() : 0;
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
