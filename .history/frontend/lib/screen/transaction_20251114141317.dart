...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadAccounts() async {
    final data = await _bankService.getAccounts();
    setState(() {
      accounts = data;
      if (accounts.isNotEmpty) selectedAccountId = accounts.first['id'];
    });
  ...
  Future<void> _loadTransactions() async {
    final allTx = await _service.getTransactions();
    setState(() {
      _transactions = allTx.reversed.take(5).toList(); // 5 giao dịch gần nhất
    });
  }
  ...
  void _saveTransaction() async {
    ...
    if (success) {
      ...
      _loadTransactions(); // cập nhật danh sách
    }
  }
          ...
          children: [
                ...
                children: [
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    items: accounts
                        .map((a) => DropdownMenuItem<int>(
                              value: a['id'],
                              child: Row(children: [Icon(Icons.account_balance), const SizedBox(width: 8), Text(a['name'])]),
                            ))
                        .toList(),
                    onChanged: (v) => setState(() => selectedAccountId = v),
                  ),
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    items: categories
                        .map((c) => DropdownMenuItem<int>(
                              value: c['id'],
                              child: Row(children: [Icon(Icons.category), const SizedBox(width: 8), Text(c['name'])]),
                            ))
                        .toList(),
                    onChanged: (v) => setState(() => selectedCategoryId = v),
                  ),
            ...
            if (_transactions.isEmpty)
              ...
              Column(
                children: _transactions.map((tx) {
                  final date = DateTime.parse(tx['date']);
                  final accountName = accounts.firstWhere((a) => a['id'] == tx['accountId'], orElse: () => {'name': 'N/A'})['name'];
                  final categoryName = categories.firstWhere((c) => c['id'] == tx['categoryId'], orElse: () => {'name': 'N/A'})['name'];
                  return Card(
                    margin: const EdgeInsets.symmetric(vertical: 4),
                    child: ListTile(
                      leading: Icon(tx['type'] == 'Chi' ? Icons.remove : Icons.add, color: Colors.orange),
                      title: Text('${tx['amount']} VND - ${tx['desc'] ?? ''}'),
                      subtitle: Text('Tài khoản: $accountName\nHạng mục: $categoryName\nNgày: ${date.day}/${date.month}/${date.year}'),
                    ),
                  );
                }).toList(),
              ),
