...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Map<String, double> singleMonthChi = {};
  Map<String, double> singleMonthThu = {};
  Map<String, double> weeklyChi = {};
  Map<String, double> weeklyThu = {};
  Map<String, double> dailyChi = {};
  Map<String, double> dailyThu = {};
  ...
  Future<void> _loadDefaultData() async {
    try {
      final report = await _service.getDefaultReport();
      setState(() {
        dailyChi = report["daily"]?["Chi"] ?? {};
        dailyThu = report["daily"]?["Thu"] ?? {};
        weeklyChi = report["weekly"]?["Chi"] ?? {};
        weeklyThu = report["weekly"]?["Thu"] ?? {};
        singleMonthChi = report["monthly"]?["Chi"] ?? {};
        singleMonthThu = report["monthly"]?["Thu"] ?? {};
        isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _pickSingleMonth() async {
    ...
    if (picked != null) {
      ...
      final monthData = await _service.getMonthReport(selectedMonth);
      setState(() {
        singleMonthChi = monthData["Chi"] ?? {};
        singleMonthThu = monthData["Thu"] ?? {};
        isLoading = false;
      });
    }
  }
  ...
  Future<void> _pickRange() async {
    ...
    if (picked != null) {
      ...
      final rangeData = await _service.getRangeReport(selectedRange!.start, selectedRange!.end);
      setState(() {
        singleMonthChi = rangeData["Chi"] ?? {};
        singleMonthThu = rangeData["Thu"] ?? {};
        isLoading = false;
      });
    }
  }
  ...
  Color categoryColor(String key, {bool isIncome = false}) {
    if (isIncome) {
      switch(key){
        case "Ăn uống": return Colors.green;
        case "Di chuyển": return Colors.blue;
        case "Hóa đơn": return Colors.teal;
        case "Mua sắm": return Colors.purple;
        case "Giải trí": return Colors.indigo;
        default: return Colors.lightGreen;
      }
    } else {
      switch(key){
        ...
        case "Di chuyển": return Colors.red;
        case "Hóa đơn": return Colors.brown;
        case "Mua sắm": return Colors.purple.shade700;
        case "Giải trí": return Colors.pink;
        default: return Colors.orange;
      }
    }
  ...
  Widget buildBarChart(Map<String,double> data,String title,{bool isIncome=false}) {
                ...
                return BarChartGroupData(
                  ...
                  barRods: [
                    BarChartRodData(
                      toY: data[key]!,
                      width: 22,
                      color: categoryColor(key,isIncome:isIncome),
                      borderRadius: BorderRadius.circular(6),
                    )
                  ],
                );
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height:20),
            buildBarChart(singleMonthChi,"Chi tiêu tháng",isIncome:false),
            buildBarChart(singleMonthThu,"Thu nhập tháng",isIncome:true),
            const SizedBox(height:24),
            buildPieChart(weeklyChi,"Chi tiêu tuần",isIncome:false),
            buildPieChart(weeklyThu,"Thu nhập tuần",isIncome:true),
            const SizedBox(height:20),
            buildPieChart(dailyChi,"Chi tiêu hôm nay",isIncome:false),
            buildPieChart(dailyThu,"Thu nhập hôm nay",isIncome:true),
          ],
        ),
      ),
    );
