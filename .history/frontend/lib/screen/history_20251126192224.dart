...
class HistoryScreen extends StatefulWidget {
  final Function(Map<String, dynamic>)? onEdit; // callback trả transaction về cha
  final VoidCallback? onUpdate; // callback sau khi xóa
  ...
  void _deleteHistory(dynamic id) async {
    ...
    if (result != null) {
      ...
      setState(() {
        ...
        double amount = double.tryParse(deleted['so_tien'].toString()) ?? 0;
        if (deleted['loai_gd'] == "Thu") totalAmount -= amount;
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              // Gọi callback trả transaction về màn hình cha
              if (widget.onEdit != null) {
                transaction['so_tien'] = double.tryParse(controller.text) ?? 0;
                widget.onEdit!(transaction);
              }
              Navigator.pop(context);
            },
