...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadAccounts() async {
    ...
    await bankService.loadAccounts();
    setState(() {
      accounts = bankService.accounts.map((e) => {
        "id": e['id'],
        "name": e['ten_tai_khoan'],
        "icon": _mapIcon(e['loai_tai_khoan']),
      }).toList();
  ...
  void _saveTransaction() async {
    ...
    final result = await TransactionService().addTransaction(
      userId: userId,
      accountId: selectedAccountId!,
      danhMucId: selectedChiCon?['id'] ?? 53,
      type: isChi ? "Chi" : "Thu",
      amount: amountCtrl.text,
      description: descCtrl.text,
      date: selectedDate.toIso8601String(),
      ...
      if (result['warning'] == true) {
        showDialog(
          ...
          builder: (_) => AlertDialog(
            ...
            content: Text("Bạn đã đạt ${result['percent']}% ngân sách tháng này!"),
            actions: [
              TextButton(
  ...
  Widget _buildToggle() { /* giữ nguyên code */ 
    return Container(
      decoration: BoxDecoration(
        color: neutralColor,
        borderRadius: BorderRadius.circular(12),
  ...
  Widget _buildImagePicker() {
    final content = kIsWeb
        ? (selectedImageBytes == null ? const Icon(Icons.camera_alt_rounded, size: 40, color: Colors.grey)
                                     : Image.memory(selectedImageBytes!, fit: BoxFit.cover))
        : (selectedImageFile == null ? const Icon(Icons.camera_alt_rounded, size: 40, color: Colors.grey)
                                     : Image.file(selectedImageFile!, fit: BoxFit.cover));
  ...
  // void _showChiMenu() async {
  //   final result = await Navigator.push(
  //     context, MaterialPageRoute(builder: (_) => CategoryScreenUI(loai: "Chi")));
  //   if (result != null) setState(() => selectedChiCon = result);
  // }
void _showChiMenu() async {
  final result = await Navigator.push(
    context,
  ...
  // void _showThuMenu() async {
  //   final result = await Navigator.push(
  //     context, MaterialPageRoute(builder: (_) => CategoryScreenUI(loai: "Thu")));
  //   if (result != null) setState(() => selectedThuLoai = result);
  // }
  void _showThuMenu() async {
  final result = await Navigator.push(
    context,
