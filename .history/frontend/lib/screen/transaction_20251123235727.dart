...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadUserId() async {
    ...
    userId = prefs.getInt('userId') ?? 1; // fallback userId = 1
    await _loadAccounts();
  ...
  Future<void> _loadAccounts() async {
    try {
      final list = await TransactionService.getAccounts(userId!);
      setState(() {
        accounts = list;
  ...
  Future<void> _loadCategories() async {
    try {
      final chi = await TransactionService.getChiCategories(userId!);
      final thu = await TransactionService.getThuCategories(userId!);
      setState(() {
        chiDanhMuc = chi;
        thuLoai = thu;
      });
    ...
    return DropdownButtonFormField<int>(
      ...
      items: accounts.map<DropdownMenuItem<int>>((account) {
        return DropdownMenuItem<int>(
          ...
          child: Row(
            children: [
              Icon(_getIconFromString(account["icon"] as String?), color: primaryColor),
              const SizedBox(width: 8),
              Text(account["name"]),
            ],
          ),
  ...
  Widget _buildTransactionList() {
    ...
    return Column(
      children: transactions.map((tx) {
        ...
        if (tx['anh_hoa_don'] != null) {
          imgWidget = kIsWeb
              ? Image.network(tx['anh_hoa_don'], width: 50, height: 50, fit: BoxFit.cover)
              : Image.file(File(tx['anh_hoa_don']), width: 50, height: 50, fit: BoxFit.cover);
        }
          ...
          child: ListTile(
            leading: CircleAvatar(
              backgroundColor: tx['loai_gd'] == "Chi"
                  ...
                  : secondaryColor.withOpacity(0.5),
              child: Icon(
                tx['loai_gd'] == "Chi" ? Icons.arrow_upward : Icons.arrow_downward,
                color: primaryColor,
              ),
            ),
            title: Text(tx['ten_danh_muc'] ?? tx['loai_gd']),
          ...
          children: [
            ...
            Flexible(
              child: ListView(
                ...
                children: chiDanhMuc.map((cat) {
                  return ExpansionTile(
                    leading: Icon(_getIconFromString(cat["icon"]), color: primaryColor),
                    title: Text(cat["name"]),
                    children: (cat["children"] as List).map((child) {
                      return ListTile(
                        leading: Icon(_getIconFromString(child["icon"]), color: Colors.grey),
                        title: Text(child["name"]),
                        onTap: () {
                          setState(() {
                            selectedChiDanhMuc = cat["name"];
          ...
          children: [
            ...
            Flexible(
              child: ListView(
                ...
                children: thuLoai.map((e) {
                  return ListTile(
                    leading: Icon(_getIconFromString(e["icon"]), color: primaryColor),
                    title: Text(e["name"]),
                    onTap: () {
                      setState(() {
