...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> saveTransaction() async {
    ...
    int categoryId = 1;
    if (isChi) {
      categoryId = chiDanhMuc
              .indexWhere((cat) => cat['name'] == selectedChiDanhMuc) +
          1;
    } else {
      categoryId =
          thuLoai.indexWhere((e) => e['name'] == selectedThuLoai) + 1;
    }
  ...
  Widget _buildForm() {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
          color: neutralColor, borderRadius: BorderRadius.circular(16)),
      child: Column(
        children: [
          _buildAmountField(),
          const SizedBox(height: 16),
