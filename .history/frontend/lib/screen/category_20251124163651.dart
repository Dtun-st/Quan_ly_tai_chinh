...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  Future<void> _loadCategories() async {
    final data = await CategoryService.getCategories(widget.title == "Chi tiêu" ? "Chi" : "Thu");
    setState(() {
      categories = data.map<Map<String, dynamic>>((e) => Map<String, dynamic>.from(e)).toList();
    });
  }
  ...
  void _addCategory({int? parentId}) async {
    ...
    await showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(parentId == null ? "Thêm ${widget.title}" : "Thêm danh mục con"),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: controller, decoration: const InputDecoration(hintText: "Tên danh mục")),
            const SizedBox(height: 12),
            ElevatedButton(
              onPressed: () async => selectedIcon = await chooseIcon(),
              child: const Text("Chọn icon"),
            ),
          ],
        ),
  ...
  void _editCategory(int id, String name, String icon) async {
    final controller = TextEditingController(text: name);
    String selectedIcon = icon;
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                final iconSel = await chooseIcon();
                if (iconSel != null) selectedIcon = iconSel;
              },
              child: const Text("Chọn icon"),
            ),
          ],
          ...
          ElevatedButton(
            onPressed: () async {
              if (controller.text.isNotEmpty) {
                await CategoryService.updateCategory(id, controller.text.trim(), selectedIcon);
                _loadCategories();
                Navigator.pop(context);
              }
            },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Danh mục ${widget.title}"), backgroundColor: Colors.deepOrange),
      floatingActionButton: FloatingActionButton(onPressed: () => _addCategory(), backgroundColor: Colors.deepOrange, child: const Icon(Icons.add)),
      body: ListView.builder(
        itemCount: categories.length,
        itemBuilder: (context, index) {
          ...
          return Card(
            margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            child: ListTile(
              leading: Icon(getIcon(icon)),
              title: Text(name, style: const TextStyle(fontSize: 18)),
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _editCategory(item['id'], name, icon)),
