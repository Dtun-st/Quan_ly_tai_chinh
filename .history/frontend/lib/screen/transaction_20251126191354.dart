...
class TransactionScreen extends StatefulWidget {
  final Map<String, dynamic>? transaction; // Nếu có thì là sửa, null là thêm mới
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void initState() {
    super.initState();
    _loadAccounts();
    if (widget.transaction != null) _loadTransaction(widget.transaction!);
  }

  void _loadTransaction(Map<String, dynamic> t) {
    setState(() {
      isChi = t['loai_gd'] == "Chi";
      amountCtrl.text = t['so_tien'].toString();
      descCtrl.text = t['mo_ta'] ?? '';
      selectedDate = DateTime.tryParse(t['ngay_giao_dich'] ?? '') ?? DateTime.now();
      selectedAccountId = t['accountId'];
      if (isChi) {
        selectedChiCon = {
          'id': t['danhMucId'],
          'ten_danh_muc': t['mo_ta'],
          'icon': 'category',
        };
      } else {
        selectedThuLoai = {
  ...
  void _saveTransaction() async {
    ...
    final tId = widget.transaction?['id'];
    bool success = false;
    ...
    if (tId != null) {
      ...
      success = await TransactionService().updateTransaction(
        ...
        userId: userId,
        accountId: selectedAccountId!,
        danhMucId: isChi ? selectedChiCon?['id'] : selectedThuLoai?['id'],
        type: isChi ? "Chi" : "Thu",
        amount: amountCtrl.text,
        description: descCtrl.text,
        date: selectedDate.toIso8601String(),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(success ? "Lưu giao dịch thành công" : "Lỗi khi lưu giao dịch"),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: backgroundColor,
      appBar: AppBar(
        title: Text(widget.transaction != null ? "Sửa giao dịch" : "Ghi chép giao dịch"),
        backgroundColor: primaryColor,
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: _saveTransaction,
