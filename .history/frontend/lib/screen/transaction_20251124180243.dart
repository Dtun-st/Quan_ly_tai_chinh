...
//   // Danh mục cứng tạm thời, có thể thay bằng API
...
//     return GestureDetector(
//       onTap: _pickDate,
//       child: _categoryFieldWidget("${selectedDate.day}/${selectedDate.month}/${selectedDate.year}", Icons.calendar_month_rounded),
//     );
//   }
...
//     final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => const CategoryScreenUI()));
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void _showChiMenu() async {
    final result = await Navigator.push(
        context,
        MaterialPageRoute(
            builder: (_) => CategoryScreenUI(loai: "Chi"))); // Chỉ hiển thị danh mục Chi
    if (result != null) setState(() => selectedChiCon = result as String);
  }
  ...
  void _showThuMenu() async {
    final result = await Navigator.push(
        context,
        MaterialPageRoute(
            builder: (_) => CategoryScreenUI(loai: "Thu"))); // Chỉ hiển thị danh mục Thu
    if (result != null) setState(() => selectedThuLoai = result as String);
  }
