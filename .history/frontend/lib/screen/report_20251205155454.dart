...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Future<void> _loadDataDefault() async {
    try {
      final data = await _service.getMonthReport(selectedMonth);

      setState(() {
        singleMonthCategories = data["categories"] ?? {};
        weeklyData = data["week"] ?? {};
        dailyData = data["day"] ?? {};
        isLoading = false;

        if (singleMonthCategories.isEmpty &&
            weeklyData.isEmpty &&
            dailyData.isEmpty) {
          errorMsg = "Chưa có dữ liệu báo cáo";
        }
    ...
    return Wrap(
      ...
      children: data.keys.map((k) {
        return Row(
          ...
          children: [
            Container(width: 14, height: 14, color: categoryColor(k)),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          buildLegend(data)  /// <<< THÊM CHÚ THÍCH
        ],
      ),
    );
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          buildBarChart(
              mode == MonthMode.single ? singleMonthCategories : rangeByMonth,
              mode == MonthMode.single
