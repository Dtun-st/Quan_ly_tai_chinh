...
// //   double? budgetMonth; // Ngân sách có thể null nếu chưa tạo
...
// //               const Text("Chưa đặt ngân sách", style: TextStyle(color: Colors.grey)),
// //               const SizedBox(height: 8),
// //               ElevatedButton(
...
// //     } else {
// //       // Ngân sách đã đặt
// //       double remaining = budgetMonth! - spentMonth;
...
// //     final controller = TextEditingController(text: budgetMonth?.toStringAsFixed(0) ?? '');
// //     showDialog(
// //       context: context,
// //       builder: (context) {
// //         return AlertDialog(
...
// //   // Phần còn lại giữ nguyên như trước (_buildUserHeader, _buildTaiKhoanList, _buildGiaoDichList, _buildThongBaoList, _buildActionButtons...)
...
//   double get tongSoDu => taiKhoan.fold(0, (sum, tk) => sum + (tk['so_du'] ?? 0));
//   double get tongThu => giaoDich.where((g) => g['loai'] == 'Thu').fold(0, (sum, g) => sum + (g['so_tien'] ?? 0));
//   double get tongChi => giaoDich.where((g) => g['loai'] == 'Chi').fold(0, (sum, g) => sum + (g['so_tien'] ?? 0));
//   double get canDoi => tongThu - tongChi;
...
class _HomePageState extends State<HomePage> {
  ...
  double get tongSoDu => taiKhoan.fold(0, (sum, tk) => sum + (double.tryParse(tk['so_du'].toString()) ?? 0));
  double get tongThu => giaoDich.where((g) => g['loai'] == 'Thu').fold(0, (sum, g) => sum + (double.tryParse(g['so_tien'].toString()) ?? 0));
  double get tongChi => giaoDich.where((g) => g['loai'] == 'Chi').fold(0, (sum, g) => sum + (double.tryParse(g['so_tien'].toString()) ?? 0));
  double get canDoi => tongThu - tongChi;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTaiKhoanList(),
            const SizedBox(height: 24),
            _buildTitleWithIcon("Giao dịch gần đây", Icons.history, onPressed: () {
              Navigator.push(
                context,
  ...
  Widget _buildTitleWithIcon(String title, IconData icon, {required VoidCallback onPressed}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
        IconButton(icon: Icon(icon, color: primaryColor), onPressed: onPressed),
      ],
    );
  }
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            title: Text(tk["ten"], style: const TextStyle(fontWeight: FontWeight.w600)),
            trailing: Text("${_formatCurrency(double.tryParse(tk["so_du"].toString()) ?? 0)} VND",
