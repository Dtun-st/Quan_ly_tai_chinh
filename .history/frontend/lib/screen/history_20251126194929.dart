...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  void _deleteHistory(dynamic id) async {
    ...
    if (result != null) {
      ...
      setState(() {
        ...
        filteredGiaoDich.removeWhere((g) => g['id'] == transactionId);
        double amount = double.tryParse(deleted['so_tien'].toString()) ?? 0;
        totalAmount += (deleted['loai_gd'] == "Thu" ? -amount : amount);
      });

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Xóa thành công và hoàn tiền về tài khoản")),
      );
    } else {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Xóa thất bại")));
  ...
  void _editTransaction(Map<String, dynamic> transaction) async {
    // Mở TransactionScreen với dữ liệu hiện tại
    ...
    _loadHistory(); // load lại sau khi sửa
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Lịch sử giao dịch"), backgroundColor: primaryColor),
      body: _loading
          ? const Center(child: CircularProgressIndicator())
          : Column(
              children: [
