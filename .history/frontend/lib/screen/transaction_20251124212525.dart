...
class _TransactionScreenState extends State<TransactionScreen> {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildDescField(),
          const SizedBox(height: 16),
          _buildImagePickerWidget(), // ← sửa ở đây
        ],
      ),
    );
  }

  // -------------- Picker UI (đã sửa Load ảnh) --------------
  Widget _buildImagePickerWidget() {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        GestureDetector(
          onTap: _pickImage, // ⬅ BẮT SỰ KIỆN CHỌN ẢNH
          child: Container(
            height: 130,
            decoration: BoxDecoration(
              color: Colors.white,
  ...
  Widget _buildAmountField() { /* giữ nguyên */ return TextField(
      controller: amountCtrl,
      keyboardType: TextInputType.number,
      style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold),
      decoration: InputDecoration(
  ...
  Widget _buildChiDanhMucField() { /* giữ nguyên */ return GestureDetector(
      onTap: _showChiMenu,
      child: _categoryFieldWidget(
        selectedChiCon != null ? selectedChiCon!['ten_danh_muc'] : "Chọn danh mục",
        selectedChiCon != null ? iconsList[selectedChiCon!['icon']] ?? Icons.category : Icons.category,
  ...
  Widget _buildThuLoaiField() { /* giữ nguyên */ return GestureDetector(
      onTap: _showThuMenu,
      child: _categoryFieldWidget(
        selectedThuLoai != null ? selectedThuLoai!['ten_danh_muc'] : "Chọn loại thu",
        selectedThuLoai != null ? iconsList[selectedThuLoai!['icon']] ?? Icons.savings_rounded : Icons.savings_rounded,
  ...
  Widget _categoryFieldWidget(String text, IconData icon) { /* giữ nguyên */ 
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 16),
      decoration: BoxDecoration(
        color: Colors.white,
  ...
  Widget _buildDescField() { /* giữ nguyên */ 
    return TextField(
      controller: descCtrl,
      maxLines: 2,
      decoration: InputDecoration(
  ...
  Widget _buildDatePicker() { /* giữ nguyên */ 
    return GestureDetector(
      onTap: _pickDate,
      child: _categoryFieldWidget(
