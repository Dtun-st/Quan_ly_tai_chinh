...
//             onPressed: () async {
//               final data = {
//                 "soTien": double.tryParse(_amountCtrl.text) ?? tx['amount'],
//                 "moTa": _descCtrl.text,
//                 "loaiGd": type,
//                 "taiKhoanId": selectedAccountId,
//                 "hanMucId": selectedCategoryId,
...
//               final ok = await _service.updateTransaction(tx['id'], data);
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void _saveOrUpdateTransaction() async {
    ...
    if (success) {
      ...
      setState(() {
        ...
        if (accounts.isNotEmpty) selectedAccountId = accounts.first['id'];
        if (categories.isNotEmpty) selectedCategoryId = categories.first['id'];
        editingTransactionId = null; // reset trạng thái sửa
      });
      _loadTransactions();
    }
  }
  ...
  void _deleteTransaction(int id) async {
    ...
    if (success) {
      _loadTransactions();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Đã xóa giao dịch'), backgroundColor: Colors.red),
      );
    }
  }
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(
                        editingTransactionId == null ? 'Lưu giao dịch' : 'Cập nhật giao dịch',
