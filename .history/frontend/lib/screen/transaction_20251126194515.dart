...
class TransactionScreen extends StatefulWidget {
  final Map<String, dynamic>? transaction; // Nếu có dữ liệu để sửa
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void initState() {
    super.initState();
    _loadAccounts().then((_) => _loadTransactionIfAny());
  }
  ...
  void _loadTransactionIfAny() {
    if (widget.transaction != null) {
      final t = widget.transaction!;
      isChi = (t['loai_gd']?.toString() ?? "Chi") == "Chi";
      amountCtrl.text = t['so_tien']?.toString() ?? '';
      descCtrl.text = t['mo_ta']?.toString() ?? '';
      selectedDate = DateTime.tryParse(t['ngay_giao_dich'] ?? '') ?? DateTime.now();
      selectedAccountId = t['accountId'] != null ? t['accountId'] as int : null;
      if (isChi) {
        selectedChiCon = t['danhMuc'] != null ? Map<String, dynamic>.from(t['danhMuc']) : null;
      } else {
        selectedThuLoai = t['danhMuc'] != null ? Map<String, dynamic>.from(t['danhMuc']) : null;
      }
    }
  }
  ...
  void _saveTransaction() async {
    ...
    final danhMucId = isChi
        ? (selectedChiCon != null ? selectedChiCon!['id'] as int : 53)
        : (selectedThuLoai != null ? selectedThuLoai!['id'] as int : 53);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(success ? "Lưu giao dịch thành công" : "Lỗi khi lưu giao dịch")),
    );
  ...
  Widget _buildChiDanhMucField() => GestureDetector(
        onTap: _showChiMenu,
        child: _categoryFieldWidget(
          selectedChiCon != null ? selectedChiCon!['ten_danh_muc'] ?? "Chọn danh mục" : "Chọn danh mục",
  ...
  Widget _buildThuLoaiField() => GestureDetector(
        onTap: _showThuMenu,
        child: _categoryFieldWidget(
          selectedThuLoai != null ? selectedThuLoai!['ten_danh_muc'] ?? "Chọn loại thu" : "Chọn loại thu",
