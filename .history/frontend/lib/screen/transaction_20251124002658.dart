...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadTransactions() async {
    ...
    try {
      final txs = await TransactionService.getTransactions(userId!);
      setState(() => transactions = txs);
    } catch (_) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Lỗi tải giao dịch")),
      );
    }
  }
  ...
  Future<void> saveTransaction() async {
    if (userId == null || selectedAccountId == null) return;

    if (isChi && selectedChiCon == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Chọn danh mục Chi")),
      );
      return;
    }
    if (!isChi && selectedThuLoai == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Chọn loại Thu")),
      );
      return;
    }
    ...
    bool success = await TransactionService.saveTransaction(
      ...
      accountId: selectedAccountId!,
      type: isChi ? "Chi" : "Thu",
      categoryName: isChi ? selectedChiCon! : selectedThuLoai!,
      amount: double.tryParse(amountCtrl.text) ?? 0,
      description: descCtrl.text,
      date: selectedDate,
      imageFile: selectedImageFile,
  ...
  Widget _buildAccountDropdown() => DropdownButtonFormField<int>(
        ...
        items: accounts
            .map((a) => DropdownMenuItem<int>(
                  value: a["id"],
                  child: Row(
                    children: [Icon(a["icon"], color: primaryColor), const SizedBox(width: 8), Text(a["name"])],
                  ),
                ))
            .toList(),
  ...
  Widget _buildChiDanhMucField() => GestureDetector(
        onTap: _showChiMenu,
        child: _categoryFieldWidget(selectedChiCon ?? "Chọn danh mục"),
      );

  Widget _buildThuLoaiField() => GestureDetector(
        onTap: _showThuMenu,
        child: _categoryFieldWidget(selectedThuLoai ?? "Chọn loại thu"),
      );

  Widget _categoryFieldWidget(String text) => Container(
        padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 16),
        decoration: BoxDecoration(
          color: Colors.white,
          border: Border.all(color: Colors.grey.shade300),
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [Text(text), const Icon(Icons.arrow_forward_ios_rounded, size: 16)],
        ),
      );

  Widget _buildDatePicker() => GestureDetector(
        onTap: _pickDate,
        child: _categoryFieldWidget("${selectedDate.day}/${selectedDate.month}/${selectedDate.year}"),
      );
  ...
  Widget _buildImagePicker() => GestureDetector(
        ...
        child: Container(
          ...
          child: Center(
            child: kIsWeb
                ? (selectedImageBytes != null ? Image.memory(selectedImageBytes!, fit: BoxFit.cover) : const Icon(Icons.camera_alt, size: 40))
                : (selectedImageFile != null ? Image.file(selectedImageFile!, fit: BoxFit.cover) : const Icon(Icons.camera_alt, size: 40)),
          ),
        ),
      );
  ...
  Widget _buildTitle(String text) => Text(text, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18));

  Widget _buildTransactionList() {
    if (transactions.isEmpty) return const Center(child: Text("Chưa có giao dịch nào"));
    return Column(
      children: transactions.map((tx) {
        final amountText = "${tx['type'] == 'Chi' ? '-' : '+'}${tx['amount']} VND";
        return Card(
          margin: const EdgeInsets.symmetric(vertical: 6),
          child: ListTile(
            leading: CircleAvatar(child: Icon(tx['type'] == 'Chi' ? Icons.arrow_upward : Icons.arrow_downward, color: primaryColor)),
            title: Text(tx['category_name'] ?? tx['type']),
            subtitle: Text("Ngày: ${tx['date'].toString().substring(0, 10)}"),
          ...
          return ExpansionTile(
            ...
            children: (cat["children"] as List).map((child) {
              return ListTile(
                ...
                title: Text(child["name"]),
                onTap: () {
                  setState(() => selectedChiCon = child["name"]);
                  Navigator.pop(context);
                },
              );
            }).toList(),
  ...
  Future<void> _pickDate() async {
    DateTime? d = await showDatePicker(context: context, initialDate: selectedDate, firstDate: DateTime(2000), lastDate: DateTime(2100));
    if (d != null) setState(() => selectedDate = d);
  }
}
