...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Widget buildMonthChart() {
    if (mode == null) {
      return const Padding(
        ...
        child: Text(
          "Vui lòng chọn tháng hoặc khoảng thời gian",
    ...
    if (mode == MonthMode.range) {
      return buildBarChart(
        rangeByMonth,
        "Khoảng: ${selectedRange!.start.day}/${selectedRange!.start.month} → "
  ...
  Widget buildBarChart(Map<String, double> data, String title, {bool useColor = true}) {
    if (data.isEmpty) {
      return const SizedBox(height: 80, child: Center(child: Text("Không có dữ liệu")));
    }
    ...
    return Column(
      children: [
        ...
        buildLegend(), /// ⭐ THÊM CHÚ THÍCH Ở ĐÂY ⭐
      ],
    );
  }
  ...
  Widget buildPie(Map<String, double> data, String title) {
    ...
    return Column(
      children: [
        ...
        buildLegend(), /// ⭐ THÊM CHÚ THÍCH Ở ĐÂY LUÔN ⭐
      ],
    );
  }
