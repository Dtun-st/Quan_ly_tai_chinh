...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void _saveTransaction() async {
    ...
    bool success = await TransactionService().addTransaction(
      userId: 1, // test user
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(success ? "Lưu thành công" : "Lỗi lưu giao dịch")),
    );
  }
  ...
  Widget _buildToggle() { /* giữ nguyên code */ return Container(
    decoration: BoxDecoration(
      color: neutralColor,
      borderRadius: BorderRadius.circular(12),
      border: Border.all(color: primaryColor.withOpacity(0.5), width: 1.5),
    ),
    child: Row(children: [_toggleItem("Chi", true), _toggleItem("Thu", false)]),
  );}

  Widget _toggleItem(String label, bool state) { /* giữ nguyên */ 
    bool active = (state == isChi);
    return Expanded(
      child: GestureDetector(
        onTap: () => setState(() => isChi = state),
  ...
  Widget _buildForm() { /* giữ nguyên */ 
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: neutralColor, borderRadius: BorderRadius.circular(16)),
  ...
  Widget _buildAmountField() { return TextField(
    controller: amountCtrl,
    keyboardType: TextInputType.number,
    style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold),
    decoration: InputDecoration(
  ...
  Widget _buildAccountDropdown() { /* giữ nguyên */ 
    return DropdownButtonFormField<int>(
      value: selectedAccountId,
      decoration: InputDecoration(
        labelText: "Tài khoản",
      ...
      items: accounts.map((acc) => DropdownMenuItem<int>(
        value: acc['id'],
        child: Row(children: [Icon(acc['icon'], color: primaryColor), const SizedBox(width: 8), Text(acc['name'])]),
      )).toList(),
      onChanged: (val) => setState(() => selectedAccountId = val),
    );
  }

  Widget _buildChiDanhMucField() { return GestureDetector(
    onTap: _showChiMenu,
    child: _categoryFieldWidget(
  ...
  Widget _buildThuLoaiField() { return GestureDetector(
    onTap: _showThuMenu,
    child: _categoryFieldWidget(
  ...
  void _showChiMenu() async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => CategoryScreenUI(loai: "Chi")));
    if (result != null && result is Map<String, dynamic>) setState(() => selectedChiCon = result);
  }

  void _showThuMenu() async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => CategoryScreenUI(loai: "Thu")));
    if (result != null && result is Map<String, dynamic>) setState(() => selectedThuLoai = result);
  }
