...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  Future<void> _loadCategories() async {
    final data =
        await service.getCategories(widget.title == "Chi tiêu" ? "Chi" : "Thu");
    setState(() {
      categories = data;
    });
  }
                ...
                ElevatedButton(
                    onPressed: () async {
                      if (controller.text.isNotEmpty && selectedIcon != null) {
                        bool success;
                        if (parentId == null) {
                          success = await service.addCategory(
                              controller.text.trim(),
                              widget.title == "Chi tiêu" ? "Chi" : "Thu");
                        } else {
                          // Thêm danh mục con backend cần mở rộng
                          // Hiện tại giả lập gọi addCategory giống cha
                          success = await service.addCategory(
                              controller.text.trim(),
                              widget.title == "Chi tiêu" ? "Chi" : "Thu");
                        }
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ExpansionTile(
              ...
              children: [
                ...((children as List).map((child) {
                  final childName = child['ten'] ?? '';
                  final childIcon = child['icon'] ?? '';
                  return ListTile(
                    leading: Icon(getIcon(childIcon)),
