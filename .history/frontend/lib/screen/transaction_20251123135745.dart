...
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Chọn danh mục Chi")));
//       return;
//     }
//     if (!isChi && selectedThuLoai == null) {
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Chọn loại Thu")));
//       return;
//     }
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _loadTransactions() async {
    ...
    try {
      final data = await TransactionService.getTransactions(1); // userId = 1
      transactions = data.map<Map<String, dynamic>>((tx) {
        return {
          "id": tx['id'],
          "type": tx['loai_gd'],
          "categoryName": tx['danh_muc_name'] ?? "Danh mục",
          "amount": tx['so_tien'].toString(),
          "date": tx['ngay_giao_dich'],
          "imageUrl": tx['anh_hoa_don'] ?? "",
          "accountId": tx['tai_khoan_id'],
          "description": tx['mo_ta'] ?? "",
        };
      }).toList();
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Không tải được giao dịch: $e")));
    } finally {
  ...
  Future<void> saveTransaction() async {
    ...
    if (editingTransactionId != null) {
      ...
      // Create
      success = await TransactionService.saveTransaction(
        userId: 1,
        accountId: selectedAccountId!,
        categoryId: 1,
        type: isChi ? "Chi" : "Thu",
  ...
  void _editTransaction(Map<String, dynamic> tx) {
    setState(() {
      ...
      amountCtrl.text = tx['amount'];
      descCtrl.text = tx['description'];
    ...
    return Row(
      ...
      children: [
        ChoiceChip(
          ...
          selected: isChi,
          onSelected: (_) => setState(() => isChi = true),
        ...
        ChoiceChip(
          ...
          selected: !isChi,
          onSelected: (_) => setState(() => isChi = false),
          selectedColor: primaryColor,
        ),
      ],
    );
  ...
  Widget _buildForm() {
    return Column(
      ...
      children: [
        ...
        ElevatedButton(
          onPressed: saveTransaction,
          child: Text(editingTransactionId != null ? "Cập nhật" : "Thêm"),
        ),
      ],
    );
  }
      ...
      children: transactions.map((tx) {
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text("Ngày: ${tx['date'].toString().split('T').first}"),
            trailing: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(
