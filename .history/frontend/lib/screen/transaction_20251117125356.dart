...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  final Map<String, List<String>> chiCon = {
    'Ăn uống': ['Nhà hàng', 'Cà phê', 'Ăn vặt'],
    'Dịch vụ sinh hoạt': ['Điện', 'Nước', 'Internet'],
    'Đi lại': ['Xe buýt', 'Xăng xe', 'Taxi'],
    'Con cái': ['Học phí', 'Quần áo', 'Đồ chơi'],
    'Trang phục': ['Áo', 'Quần', 'Giày dép'],
    'Hiếu hỉ': ['Đám cưới', 'Tang lễ'],
    'Sức khỏe': ['Khám bệnh', 'Thuốc men'],
    'Nhà cửa': ['Sửa chữa', 'Nội thất'],
    'Hưởng thụ': ['Du lịch', 'Giải trí'],
    'Phát triển bản thân': ['Khóa học', 'Sách'],
  };

  // Dữ liệu cứng Thu
  final List<String> thuLoai = ['Lương', 'Thưởng', 'Được biếu', 'Lãi'];

  // Ví
  final List<String> viList = ['Tiền mặt', 'Ngân hàng', 'Ví điện tử'];

  // Giá trị được chọn
  String? selectedChiDanhMuc;
  String? selectedChiCon;
  String? selectedThuLoai;
  String? selectedViChi;
  String? selectedViThu;
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Card(
                  ...
                  child: ListTile(
                    leading: Icon(
                      isChi ? Icons.remove : Icons.add,
                      color: isChi ? Colors.red : Colors.green,
                    ),
                    title: Text(isChi
                        ? '150,000 VND - Nhà hàng'
                        : '5,000,000 VND - Lương'),
                    subtitle: const Text('Ngày: 10/11/2025'),
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        IconButton(
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (v) {
            setState(() {
              selectedChiDanhMuc = v;
              selectedChiCon = null;
            });
          },
        ),
        const SizedBox(height: 8),
        if (selectedChiDanhMuc != null && chiCon[selectedChiDanhMuc!] != null)
          DropdownButtonFormField<String>(
            value: selectedChiCon,
            hint: const Text('Chọn chi tiết'),
            items: chiCon[selectedChiDanhMuc!]!
                .map((e) => DropdownMenuItem(value: e, child: Text(e)))
                .toList(),
            onChanged: (v) => setState(() => selectedChiCon = v),
          ),
        const SizedBox(height: 12),
        ...
        DropdownButtonFormField<String>(
          ...
          items:
              viList.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
          onChanged: (v) => setState(() => selectedViChi = v),
        ),
        const SizedBox(height: 12),
        const Text('Ngày', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items:
              viList.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
          onChanged: (v) => setState(() => selectedViThu = v),
        ),
        const SizedBox(height: 12),
        const Text('Ngày', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
