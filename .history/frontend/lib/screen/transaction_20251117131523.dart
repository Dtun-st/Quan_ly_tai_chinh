...
// Dữ liệu cứng mẫu
const List<Map<String, dynamic>> chiDanhMuc = [
  {
    "name": "Ăn uống",
    "icon": Icons.restaurant,
    "children": ["Nhà hàng", "Cà phê", "Ăn vặt"]
  },
  {
    "name": "Dịch vụ sinh hoạt",
    "icon": Icons.home_repair_service,
    "children": ["Điện", "Nước", "Internet"]
  },
  {
    "name": "Đi lại",
    "icon": Icons.directions_car,
    "children": ["Xe buýt", "Xăng xe", "Taxi"]
  },
];

const List<Map<String, dynamic>> thuLoai = [
...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void _selectChiDanhMuc() async {
    // Mở bottom sheet để chọn danh mục
                  ...
                  children: chiDanhMuc.map((cat) {
                    return ExpansionTile(
                      ...
                      children: [
                        ...
                        ...List.generate(cat['children'].length, (i) {
                          final child = cat['children'][i];
                          return ListTile(
                            title: Text(child),
                            onTap: () {
                              setState(() {
                                selectedChiDanhMuc = cat['name'];
                                selectedChiCon = child;
                              });
                              Navigator.pop(ctx);
                            },
                          );
  ...
  Widget _buildCategoryBox(String title, IconData icon, String? selectedText, VoidCallback onTap) {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text(selectedText ?? title)),
  ...
  Widget _buildFormChi() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildAmountField(),
        const SizedBox(height: 12),
        _buildCategoryBox('Danh mục', Icons.category,
            selectedChiCon ?? selectedChiDanhMuc, _selectChiDanhMuc),
        const SizedBox(height: 12),
  ...
  Widget _buildFormThu() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildAmountField(),
        const SizedBox(height: 12),
        _buildCategoryBox('Loại thu', Icons.monetization_on, selectedThuLoai, _selectThuLoai),
        const SizedBox(height: 12),
        _buildAccountBox(),
        const SizedBox(height: 12),
        _buildDatePicker(),
  ...
  Widget _buildAccountBox() {
    return _buildCategoryBox(
        'Chọn tài khoản', Icons.account_balance_wallet, accounts.firstWhere((a) => a['id'] == selectedAccountId)['name'], () {});
  }
  ...
  Widget _buildFrequentlyUsed() {
    final List<Map<String, dynamic>> frequentlyUsed = [
      ...
      {"name": "Xăng xe", "icon": Icons.local_gas_station, "color": Colors.orange},
    ];
    return SizedBox(
      height: 80,
      child: ListView(
        scrollDirection: Axis.horizontal,
        children: frequentlyUsed.map((e) {
          return Container(
            width: 70,
            ...
            child: Column(
              children: [
                ...
                Text(e['name'], textAlign: TextAlign.center, style: const TextStyle(fontSize: 12)),
              ],
            ),
          );
        }).toList(),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: Center(
                  child: Text(
                'Chi',
                style: TextStyle(color: isChi ? Colors.white : Colors.black, fontWeight: FontWeight.bold),
              )),
            ),
          ),
        ),
  ...
  Widget _buildTransactionList() {
    return const SizedBox(); // Giao diện tạm, vì yêu cầu cứng
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            isChi ? _buildFormChi() : _buildFormThu(),
          ],
        ),
      ),
      bottomNavigationBar: const CustomBottomNav(currentIndex: 2),
