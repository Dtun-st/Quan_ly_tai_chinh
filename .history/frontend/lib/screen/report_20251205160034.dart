...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Future<void> _loadMonthData() async {
    ...
    try {
      final data = await _service.getMonthReport(selectedMonth);
      setState(() {
        singleMonthCategories = data;
        isLoading = false;
        if (data.isEmpty) errorMsg = "Chưa có dữ liệu báo cáo";
      });
    } catch (e) {
      setState(() { isLoading = false; errorMsg = "Lỗi tải dữ liệu: $e"; });
    }
  }
  ...
  Future<void> _loadRangeData() async {
    ...
    setState(() { isLoading = true; errorMsg = null; });
    try {
      final data = await _service.getRangeReport(selectedRange!.start, selectedRange!.end);
      setState(() {
        rangeByMonth = data;
        isLoading = false;
        if (data.isEmpty) errorMsg = "Chưa có dữ liệu báo cáo";
        ...
        SizedBox(height: 230, child: BarChart(BarChartData(
          ...
          barGroups: List.generate(data.length, (i){
            ...
            return BarChartGroupData(x:i, barRods:[BarChartRodData(
              toY:data[key]!, width:22, color: useColor?categoryColor(key):Colors.deepOrange, borderRadius: BorderRadius.circular(6)
            )]);
          }),
          titlesData: FlTitlesData(
            ...
            bottomTitles: AxisTitles(sideTitles: SideTitles(
              ...
              getTitlesWidget:(v,meta){
                ...
                return Padding(padding: const EdgeInsets.only(top:6), child: Text(key, style: const TextStyle(fontSize:12)));
              }
            ))
          )
        )))
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Báo cáo chi tiêu"), backgroundColor: Colors.deepOrange, centerTitle: true),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children:[
            Row(children:[
              ElevatedButton(onPressed:_pickSingleMonth,
                style: ElevatedButton.styleFrom(backgroundColor:mode==MonthMode.single?Colors.deepOrange:Colors.grey.shade300,
                  foregroundColor: mode==MonthMode.single?Colors.white:Colors.black),
                child: const Text("Chọn tháng")
              ),
              const SizedBox(width:8),
              ElevatedButton(onPressed:_pickRange,
                style: ElevatedButton.styleFrom(backgroundColor:mode==MonthMode.range?Colors.deepOrange:Colors.grey.shade300,
                  foregroundColor: mode==MonthMode.range?Colors.white:Colors.black),
                child: const Text("Khoảng thời gian")
              ),
            ]),
            const SizedBox(height:20),
            mode==MonthMode.single?buildBarChart(singleMonthCategories,"Chi tiêu tháng ${monthLabel(selectedMonth)}")
            :buildBarChart(rangeByMonth, selectedRange==null?"Chưa chọn khoảng thời gian":"Từ ${selectedRange!.start.day}/${selectedRange!.start.month} → ${selectedRange!.end.day}/${selectedRange!.end.month}", useColor:false)
          ]
        )
      )
    );
