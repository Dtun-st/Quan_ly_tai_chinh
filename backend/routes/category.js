...
router.get('/:userId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Có lỗi xảy ra khi lấy danh mục' });
  }
});
...
router.post('/', async (req, res) => {
  ...
  if (!nguoi_dung_id) {
    return res.status(400).json({ success: false, message: "Thiếu thông tin người dùng (userId)" });
  }
  ...
  try {
    ...
    res.json({ success: true, message: 'Danh mục đã được thêm thành công' });
  } catch (err) {
    console.error('❌ Lỗi khi thêm danh mục:', err);
    res.status(500).json({ success: false, message: 'Có lỗi xảy ra khi thêm danh mục' });
  }
});
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!rows[0].nguoi_dung_id)
      return res.status(403).json({ success: false, message: "Danh mục hệ thống không thể sửa" });

    await pool.execute(
      'UPDATE danh_muc SET ten_danh_muc=?, icon=? WHERE id=?',
      [ten_danh_muc, icon, id]
    );
    res.json({ success: true, message: 'Danh mục đã được cập nhật thành công' });
  } catch (err) {
    console.error('❌ Lỗi khi sửa danh mục:', err);
    res.status(500).json({ success: false, message: 'Có lỗi xảy ra khi cập nhật danh mục' });
  }
});
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!rows[0].nguoi_dung_id)
      return res.status(403).json({ success: false, message: "Danh mục hệ thống không thể xóa" });

    await pool.execute('DELETE FROM danh_muc WHERE id=?', [id]);
    res.json({ success: true, message: 'Danh mục đã được xóa thành công' });
  } catch (err) {
    console.error('❌ Lỗi khi xóa danh mục:', err);
    res.status(500).json({ success: false, message: 'Có lỗi xảy ra khi xóa danh mục' });
  }
});
